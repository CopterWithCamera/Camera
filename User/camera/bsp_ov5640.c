...
//    0x380b, 0x78, // DVPVO
	...
	//自动调整分辨率
	0x3808, (0xFF00 & IMG_WIDTH) >> 8, 		// DVPHO
    0x3809, 0xFF & IMG_WIDTH, 				// DVPHO
    0x380a, (0xFF00 & IMG_HEIGHT) >> 8, 	// DVPVO
    0x380b, 0xFF & IMG_HEIGHT, 				// DVPVO
...
void OV5640_DMA_Config(uint32_t DMA_Memory0BaseAddr,uint16_t DMA_BufferSize)
{
  ...
  DMA_InitStructure.DMA_Priority = DMA_Priority_VeryHigh;				//仲裁器优先级为veryhigh
...
void OV5640_RGB565Config(void)
{
	...
	switch(ImageFormat)
	{
		case BMP_AUTO:
		{
			...
			{
				OV5640_WriteReg(RGB565_Auto[i][0], RGB565_Auto[i][1]);
			}
			break;
		}
		default:
